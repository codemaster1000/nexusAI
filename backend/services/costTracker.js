...
function estimateCallCost({ model, inputTokens = 0, outputTokens = 0, explicitCostUsd }) {
...
function summarizeCost(calls) {
  const totalInputTokens = calls.reduce((acc, call) => acc + (call.inputTokens || 0), 0);
  const totalOutputTokens = calls.reduce((acc, call) => acc + (call.outputTokens || 0), 0);
